...
class ProjectConfigurationTest {
    ...
    fun setUp() {
        ...
        // Initialize project repository (same pattern as MedresLoginActivity)
    ...
    /**
     * Helper to simulate manualConfigureProject logic from MedresLoginActivity.
    ...
    fun `dev server IP is saved to preferences`() {
        ...
        val devPrefs = context.getSharedPreferences("medres_dev_prefs", Context.MODE_PRIVATE)
        
        // When: Dev server IP is saved
        val testIp = "192.168.1.100:8383"
        devPrefs.edit()
            .putString(edu.aiims.medresodk.auth.utils.MedresConstants.KEY_DEV_SERVER_IP, testIp)
            .apply()
        
        // Then: It should be retrievable
        val savedIp = devPrefs.getString(edu.aiims.medresodk.auth.utils.MedresConstants.KEY_DEV_SERVER_IP, null)
    ...
    fun `dev server IP can be cleared`() {
        ...
        val devPrefs = context.getSharedPreferences("medres_dev_prefs", Context.MODE_PRIVATE)
        devPrefs.edit()
            .putString(edu.aiims.medresodk.auth.utils.MedresConstants.KEY_DEV_SERVER_IP, "10.0.0.1:8080")
            .apply()
    ...
    fun `dev server IP is only used in DEBUG builds`() {
        ...
        // The RELEASE behavior is enforced by BuildConfig.DEBUG checks in MedresLoginActivity.
        
        // Verify this test runs in DEBUG mode
        assertThat("Tests should run in DEBUG mode", edu.aiims.medresodk.auth.BuildConfig.DEBUG, equalTo(true))
        
        // In DEBUG mode, the dev server IP is accessible and usable
        val devPrefs = context.getSharedPreferences("medres_dev_prefs", Context.MODE_PRIVATE)
        devPrefs.edit()
            .putString(edu.aiims.medresodk.auth.utils.MedresConstants.KEY_DEV_SERVER_IP, "dev.local:8080")
            .apply()
    ...
    /**
     * Simple Settings implementation for testing (mirrors MedresLoginActivity.MedresSettings)
     */
