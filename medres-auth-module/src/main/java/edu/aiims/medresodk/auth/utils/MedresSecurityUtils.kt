 ...
 */
class MedresSecurityUtils private constructor(
    private val context: Context
) {

    companion object {
        @Volatile
        private var INSTANCE: MedresSecurityUtils? = null

        fun getInstance(context: Context): MedresSecurityUtils {
            return INSTANCE ?: synchronized(this) {
                INSTANCE ?: createInstance(context.applicationContext).also { INSTANCE = it }
            }
        }

        private fun createInstance(context: Context): MedresSecurityUtils {
            return MedresSecurityUtils(context)
        }
    ...
    fun storeKeyInKeystore(alias: String, key: SecretKey): Boolean {
        return try {
            ...
            android.util.Log.e(MedresConstants.TAG_AUTH, "Failed to store key in keystore", e)
            false
        }
    }
    ...
    fun getKeyFromKeystore(alias: String): SecretKey? {
        return try {
            ...
            android.util.Log.e(MedresConstants.TAG_AUTH, "Failed to get key from keystore", e)
            null
        }
    }
    ...
    fun showBiometricPrompt(
        ...
        title: String = MedresConstants.BIOMETRIC_PROMPT_TITLE,
        subtitle: String = MedresConstants.BIOMETRIC_PROMPT_SUBTITLE,
        onSuccess: () -> Unit,
        onFailure: () -> Unit = {},
        onError: (String) -> Unit = {}
    ) {
        val promptInfo = BiometricPrompt.PromptInfo.Builder()
            ...
            .setNegativeButtonText(MedresConstants.BIOMETRIC_PROMPT_NEGATIVE)
    ...
    fun validatePinStrength(pin: String): PinStrength {
        return when {
            pin.length < MedresConstants.PIN_LENGTH_MIN -> PinStrength.TOO_SHORT
            pin.length > MedresConstants.PIN_LENGTH_MAX -> PinStrength.TOO_LONG
