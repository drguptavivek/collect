...
class RealAuthClient internal constructor(
    ...
    internal val authManager: MedresAuthManager? = null
) : AuthClient {
    ...
    private fun getRetrofit(): Retrofit {
        return retrofit ?: synchronized(this) {
            ...
            authManager?.let { am: MedresAuthManager ->
                clientBuilder.addInterceptor(AuthInterceptor(
                    am,
                    MedresTokenProvider.getInstance(context)
                ))
            }
    ...
    override suspend fun login(projectId: String, username: String, password: String, deviceId: String, comments: String?): AuthResult {
        return withContext(Dispatchers.IO) {
            try {
                Log.d("MedresAuthClient", "Attempting login for user: $username on project: $projectId device: $deviceId")
            MedresAppAnalytics.logLoginAttempt()
            ...
            // Create login request
                ...
                Log.d("MedresAuthClient", "Making API call to ${getApiService()}")
                // Make API call
                val response = getApiService().login(projectId, request)

                Log.d("MedresAuthClient", "Response code: ${response.code()}")

                if (response.isSuccessful) {
                    val body = response.body()
                    if (body != null) {
                        Log.d("MedresAuthClient", "Login successful")
                    MedresAppAnalytics.logLoginSuccess()
                    ...
                    // Parse server time from response body (ISO 8601 format)
                        ...
                        Log.e("MedresAuthClient", "Empty response body")
                        AuthResult.Error("Empty response from server")
                    }
                } else {
                    // Handle HTTP errors
                Log.e("MedresAuthClient", "HTTP Error: ${response.code()}")
                when (response.code()) {
                    400 -> {
                        MedresAppAnalytics.logLoginFailed("invalid_request_400")
                        AuthResult.Error("Invalid request")
                    }
                    401 -> {
                        MedresAppAnalytics.logLoginFailed("invalid_credentials_401")
                        AuthResult.Error("Invalid credentials")
                    }
                    403 -> {
                         MedresAppAnalytics.logLoginFailed("forbidden_403")
                         AuthResult.Error("Access forbidden")
                    }
                    404 -> {
                        MedresAppAnalytics.logLoginFailed("not_found_404")
                    ...
                    429 -> {
                        ...
                        MedresAppAnalytics.logLoginFailed("rate_limited")
                        AuthResult.Error("Too many login attempts. Please try again in $retryMinutes minutes.")
                    }
                    500 -> {
                         MedresAppAnalytics.logLoginFailed("server_error_500")
                         AuthResult.Error("Server error")
                    }
                    else -> {
                        MedresAppAnalytics.logLoginFailed("http_${response.code()}")
                        AuthResult.Error("Login failed: HTTP ${response.code()}")
                    }
                }
                }
            } catch (e: java.net.SocketTimeoutException) {
                Log.e("MedresAuthClient", "Login timeout: ${e.message}", e)
                MedresAppAnalytics.logNetworkError("timeout")
                AuthResult.Error("Connection timed out. Please check your network and try again.")
            } catch (e: java.net.UnknownHostException) {
                Log.e("MedresAuthClient", "Login DNS error: ${e.message}", e)
                MedresAppAnalytics.logNetworkError("dns_error")
                AuthResult.Error("Cannot reach server. Please check your internet connection.")
            } catch (e: java.net.ConnectException) {
                Log.e("MedresAuthClient", "Login connection refused: ${e.message}", e)
                MedresAppAnalytics.logNetworkError("connection_refused")
                AuthResult.Error("Cannot connect to server. Please try again later.")
            } catch (e: java.io.IOException) {
                Log.e("MedresAuthClient", "Login IO error: ${e.message}", e)
                MedresAppAnalytics.logNetworkError("io_error")
                AuthResult.Error("Network error. Please check your connection and try again.")
            } catch (e: Exception) {
                Log.e("MedresAuthClient", "Login exception: ${e.message}", e)
                MedresAppAnalytics.logLoginFailed("exception")
    ...
    private fun parseIsoDateTime(isoString: String): Long? {
        return try {
            ...
            try {
                ...
                Log.w("MedresAuthClient", "Failed to parse ISO datetime: $isoString")
                null
            }
        }
    }
    ...
    private fun extractServerTimeFromResponse(response: retrofit2.Response<*>): Long? {
        ...
        return try {
            ...
            Log.w("MedresAuthClient", "Failed to parse server Date header: $dateHeader")
            null
        }
    }
            ...
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    if (body?.success == true) {
                        Log.d("MedresAuthClient", "Session revoked: $userId")
                        true
                    } else {
                        Log.e("MedresAuthClient", "Revoke failed")
                        false
                    }
                } else {
                    Log.e("MedresAuthClient", "Revoke HTTP error ${response.code()}")
                    false
                }
            } catch (e: Exception) {
                Log.e("MedresAuthClient", "Revoke exception: ${e.message}", e)
                false
            }
        }
    }
    ...
    override suspend fun checkReachability(): Boolean = withContext(Dispatchers.IO) {
        ...
        reachabilityMutex.withLock {
            if (lastReachabilityResult != null && (now - lastReachabilityCheckTime) < REACHABILITY_TTL_MS) {
                Log.d("MedresAuthClient", "Returning cached reachability result: $lastReachabilityResult")
        ...
        reachabilityMutex.withLock {
            ...
            try {
                ...
                Log.d("MedresAuthClient", "Reachability check to $url returned: $responseCode (Reachable: $reachable)")
                ...
                Log.e("MedresAuthClient", "Reachability check failed", e)
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    Log.e("MedresAuthClient", "Telemetry failed: ${response.code()}")
                    when (response.code()) {
                ...
                Log.e("MedresAuthClient", "Telemetry exception: ${e.message}", e)
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    Log.e("MedresAuthClient", "Fetch project failed: ${response.code()} Body: $errorBody")
                    null
                }
            } catch (e: Exception) {
                Log.e("MedresAuthClient", "Fetch project exception: ${e.message}", e)
                null
            }
        }
    }
    ...
    companion object {
        ...
        @Volatile
        private var lastAuthManager: MedresAuthManager? = null

        fun getInstance(context: Context, apiUrl: String, authManager: MedresAuthManager? = null): RealAuthClient {
