# Debugging Guide - AIIMS ODK Collect

> Last Updated: 2025-12-28

This guide covers debugging tools and techniques for AIIMS ODK Collect development.
...
```bash
# All AIIMS authentication logs
adb logcat | grep -E "AiimsAuth|AiimsAppLock|AiimsPin|AiimsLogin"
...
# Errors for AIIMS package
adb logcat *:E | grep "aiims"
```
...
adb shell run-as org.aiims.odk.collect cat /data/data/org.aiims.odk.collect/shared_prefs/meta_prefs.xml | grep current_project_id
```
...
adb shell run-as org.aiims.odk.collect cat /data/data/org.aiims.odk.collect/shared_prefs/general_prefs*.xml
```
...
adb shell run-as org.aiims.odk.collect cat /data/data/org.aiims.odk.collect/shared_prefs/aiims_auth_prefs.xml | grep pin
```
...
**Debug Steps**:
1. Check if `AiimsAppLock` is registered:
   ```bash
...
**Solution**: Ensure `AiimsAppLock` is registered in `Collect.onCreate()`
...
1. Check token exists:
   ```bash
   adb shell run-as org.aiims.odk.collect cat /data/data/org.aiims.odk.collect/shared_prefs/aiims_auth_prefs.xml | grep auth_token
   ```
...
1. Verify build type:
   ```bash
   adb shell dumpsys package org.aiims.odk.collect | grep versionName
   # Should end with "-AIIMS-DEBUG"
   ```
2. Check DNS mapping:
   ```bash
...
|------|---------------|---------|
| `AiimsAuthManager.kt` | `login()` | Entry point for auth |
| `AiimsAuthManager.kt` | `refreshState()` | State transition logic |
| `AiimsAppLock.kt` | `onActivityStarted()` | PIN trigger logic |
...
1. Set breakpoint in `AiimsAuthManager.login()`
...
1. Set breakpoint in `AiimsAppLock.onActivityStarted()`
...
```bash
# Enter SQLite shell
adb shell run-as org.aiims.odk.collect sqlite3 /data/data/org.aiims.odk.collect/databases/odk.sqlite
...
```bash
# Browse form files
adb shell run-as org.aiims.odk.collect ls -la /data/data/org.aiims.odk.collect/files/

# View instance directories
adb shell run-as org.aiims.odk.collect ls -la /data/data/org.aiims.odk.collect/instances/

# Copy file from device
adb pull /data/data/org.aiims.odk.collect/files/forms/form.xml .
```
...
2. Select process: `org.aiims.odk.collect`
3. Profile:
   - **CPU**: Method tracing
...
adb shell run-as org.aiims.odk.collect rm /data/data/org.aiims.odk.collect/shared_prefs/aiims_auth_prefs.xml
```
...
```bash
# Set expiry to past
adb shell run-as org.aiims.odk.collect
sqlite3 /data/data/org.aiims.odk.collect/shared_prefs/aiims_auth_prefs.xml
# Edit expires_at to past timestamp
```

Or via code (debug only):
```kotlin
AiimsAuthManager.setDebugExpiry(Instant.now().minusSeconds(3600))
```
...
adb shell run-as org.aiims.odk.collect
rm /data/data/org.aiims.odk.collect/shared_prefs/aiims_auth_prefs.xml
