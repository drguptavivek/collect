...
This document details the security protection mechanisms implemented in the AIIMS ODK Collect fork. These safeguards ensure data integrity, session security, and resilience in field environments.
...
- **References**: `collect-a55`, `AiimsAuthManagerTest.kt#grace period is re-evaluated when network restores`.
...
- **References**: `collect-s0d`, `collect-0kd`, `AiimsAppLockTest.kt`.
...
- **Implementation**: The `AiimsAuthManager` detects if the user ID has changed during a login or re-auth event.
...
- **References**: `collect-3yt`, `AiimsAuthManagerTest.kt#login clears PIN when user changed`.

## 4. Project & QR Bypass Protection
**Scenario**: Using the standard ODK "Add Project" feature or scanning a generic ODK QR code to bypass the AIIMS authenticated environment.

- **Protection**: **Hardened Entry Points**.
- **Implementation**: The ODK "Projects" modal and "Add Project" UI elements are disabled. All QR code scanning is intercepted by the AIIMS Auth Layer.
- **Outcome**: Standard QR codes that omit AIIMS-specific metadata are rejected or redirected to the AIIMS Login flow, ensuring no unauthenticated "backdoors" exist.
...
- **References**: `collect-40v`, `AiimsAuthManagerTest.kt#logout cancels reachability check`, `AiimsAuthManagerTest.kt#logout handles storage failure`.
...
- **References**: `collect-49l`, `AiimsAuthManagerTest.kt#submitTelemetry queues on 401 AuthError`.
...
- **Implementation**: The `AiimsAuthManager` implements a tiered rate-limiting window for sensitive API calls.
...
- **Implementation**: All sensitive state transitions in `AiimsAuthManager` are protected by a shared Kotlin Mutex.
...
- **References**: `collect-63g`, `AiimsAuthManagerTest.kt#logout cancels reachability check`.
...
- **References**: `collect-40v`, `AiimsAuthManagerCorruptionTest.kt`.
