...
The AIIMS ODK Collect telemetry system is designed for high-fidelity event tracking in offline-first environments. It ensures that security events and device health metrics are captured and eventually synchronized with the server, even with intermittent connectivity.
...
- **Implementation**: The system uses a dedicated Room database (`AiimsDatabase`) with a `TelemetryEntity` table.
...
- **References**: `collect-5zw`, `AiimsAuthManager#submitTelemetry`.
...
- **Manager**: `AiimsAuthManager.submitTelemetry(...)`
- **Worker**: `org.aiims.odk.auth.workers.TelemetryWorker`
