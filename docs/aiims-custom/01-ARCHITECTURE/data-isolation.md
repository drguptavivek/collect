...
2.  **AIIMS Regular Storage** (`aiims_auth_prefs`): Standard SharedPreferences for non-sensitive global state and project-specific metadata.
...
- **`central_to_odk_$pid`**: Maps an AIIMS/Central Project ID to the internal ODK Project UUID.
...
> [!IMPORTANT]
> **Hardware-Backed Encryption**: All sensitive data (tokens, PINs) are stored in `aiims_auth_secure` using Android's `EncryptedSharedPreferences`.
...
1. **Login**: Token received from server, stored in `AiimsSecureStorage` (encrypted). `project_id` stored alongside for validation.
...
3. **Logout**: `AiimsAuthStorage.clearAuthData()` called. All sensitive data cleared from encrypted storage. Non-sensitive metadata (like project names) preserved for project switching history.
