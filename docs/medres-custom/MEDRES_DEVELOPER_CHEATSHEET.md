# MEDRES ODK Collect - Developer Cheat Sheet
...
### Logs
*   Filter Logcat for `MedresAuthManager` or `MedresAppLock` to see authentication state transitions and PIN lock events.

## 📦 Build Configuration

### Gradle Modules
*   `:collect_app`: The main app module.
*   `:medres-auth-module`: Contains all custom logic (Auth, PIN, API).
