# MEDRES Reauthentication Workflow - Complete Scenario Matrix
> Last Updated: 2025-12-28
> Reviewed At: 2025-12-28

This document provides a comprehensive analysis of all possible scenarios in the MEDRES authentication module's reauthentication workflow with grace period.
...
@docs/medres-custom/audit/REAUTHENTICATION_SCENARIOS.md and 
@docs/medres-custom/01-ARCHITECTURE/ 
Think carefully and propose a fix. 
Confirm first before applying fix. 
Verify. Doublecheck code.
Then Do an 
./gradlew clean
./gradlew compileMedresDebug 
./gradlew installMedresDebug

Then ask for User verofciation. 
Once user confirms, DoubleCheck code.
Update the @docs/medres-custom/audit/REAUTHENTICATION_SCENARIOS.md  and @docs/medres-custom/01-ARCHITECTURE/ . 
Finally  git commit. 
...
**Code Flow:**
```kotlin
// MedresAuthManager:checkTokenExpiry()
...
2. `MedresAppLock` triggers `MedresLoginActivity` with `is_reauth=true`
...
**Code Flow:**
```kotlin
// MedresAppLock:onActivityResumed()
...
if (authManager.isSoftExpiry.value) {
    ...
    startActivity(MedresLoginActivity with is_reauth=true)
}
```
...
**System Behavior:**
```kotlin
// MedresLoginActivity: onClick work offline button
...
**System Behavior:**
```kotlin
// MedresLoginActivity: Login success
...
### Key Files
- `MedresAuthManager.kt:checkTokenExpiry()` - Main logic
- `RealAuthClient.kt:checkReachability()` - Server check
- `MedresAppLock.kt:onActivityResumed()` - Trigger point
- `MedresLoginActivity.kt` - Reauth UI
...
**Priority Flow:**
```kotlin
// MedresAppLock evaluates in order:
if (isSoftExpiry) {
...
**Behavior:**
```kotlin
// MedresAppLock:onActivityResumed() triggered
...
```kotlin
// ClockValidator usage in MedresAuthManager:
...
- `MedresSecureStorage.kt` - Added clock validation properties
- `MedresAuthManager.kt` - Integrated ClockValidator
- `MedresConstants.kt` - Added clock validation constants
...
**Original Issue:**
- Tokens were stored in plain SharedPreferences in `MedresAuthManager`
- `MedresSecureStorage` existed but was not being used

**Resolution:**
1. Updated `MedresAuthManager` to use `MedresAuthStorage` (wraps `MedresSecureStorage`) for sensitive data
2. Added `projectId` to encrypted storage for token validation
3. Updated Dagger dependency injection to provide `MedresAuthStorage`

**Current Implementation:**
```kotlin
// Tokens now stored in encrypted storage via MedresSecureStorage
...
- `MedresAuthManager` constructor calls `refreshState()` immediately during Dagger injection
...
**Files Modified:**
- `MedresSecureStorage.kt` - Added `projectId`, StrictMode suppression, removed user auth requirement
- `MedresAuthStorage.kt` - Added `projectId` property
- `MedresAuthManager.kt` - Uses encrypted storage for tokens
- `DaggerSetup.kt` - Provides `MedresAuthStorage` dependency
- `MedresConstants.kt` - Added `KEY_PROJECT_ID` constant

**Testing:**
- Build verified: `./gradlew :collect_app:assembleMedresDebug` successful
...
```kotlin
// Activity stack:
[MedresLoginActivity (reauth)]
[PhoneCallActivity]
[MedresLoginActivity (reauth)] // Duplicate!
```
...
3. Redirected to MedresLoginActivity
...
9. MedresAppLock sees LOGGED_IN but PIN not set → does nothing
10. USER ACCESSES APP WITHOUT PIN!
```

**Root Cause:**
1. **Primary**: `MedresAppLock.kt:60-62` - When `authState == LOGGED_IN` but PIN NOT set, no action taken
...
2. **Updated `MedresLoginActivity.attemptLogin()`**: Set correct auth state based on reauth flag
```kotlin
if (isReauthMode) {
...
3. **Updated `MedresAppLock.onActivityStarted()`**: Handle `LOGGED_IN_REQUIRES_PIN`
```kotlin
...
override fun onBackPressed() {
    ...
    AlertDialog.Builder(this)
        .setTitle(getString(R.string.medres_pin_required_title))
        .setMessage(getString(R.string.medres_pin_required_message))
        .setPositiveButton(getString(R.string.medres_button_ok), null)
        .setCancelable(false)
        .show()
}
```
...
**Files Changed:**
- `medres-auth-module/src/main/java/edu.aiims.medresodk.auth/managers/MedresAuthManager.kt` - Added `LOGGED_IN_REQUIRES_PIN` to AuthState enum
- `medres-auth-module/src/main/java/edu.aiims.medresodk.auth/activities/MedresLoginActivity.kt` - Set correct state based on `EXTRA_IS_REAUTH` flag
- `medres-auth-module/src/main/java/edu.aiims.medresodk.auth/utils/MedresAppLock.kt` - Handle `LOGGED_IN_REQUIRES_PIN` (force PIN setup)
- `medres-auth-module/src/main/java/edu.aiims.medresodk.auth/activities/SetupPinActivity.kt` - Added `onBackPressed()` override
- `medres-auth-module/src/main/res/values/strings.xml` - Added dialog strings

**Verification:**
- Build verified: `./gradlew assembleMedresDebug` ✅
- BACK button: Shows "PIN Required" dialog, prevents exit ✅
- HOME button: `MedresAppLock` redirects back to PIN setup on resume ✅
