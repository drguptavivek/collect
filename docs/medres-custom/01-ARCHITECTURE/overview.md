# MEDRES ODK Collect - Architecture Overview

> Last Updated: 2025-12-31
> Reviewed At: 2025-12-31

This document provides a high-level architecture overview of the MEDRES customizations to ODK Collect.

---

## Executive Summary

The MEDRES fork implements four major modifications to standard ODK Collect:
...
graph TB
    ...
    subgraph "Storage"
        AuthMetadata["medres_auth_prefs<br/>(User, API URL, Names)"]
        AuthSecure["medres_auth_secure<br/>(Tokens, Expiry, PIN)"]
...
sequenceDiagram
    participant User
    participant Login as MedresLoginActivity
    ...
    AuthMgr->>AuthMgr: Save to medres_auth_prefs
...
graph TD
    subgraph "Logout Flow"
        Logout[User Logs Out] --> ClearAuth[MedresAuthManager.logout]
...
|-----------|---------------|
| `MedresAuthManager` | Central hub for auth state, login/logout, token retrieval |
| `MedresLoginActivity` | Login UI, project detection, manual config, re-auth flow |
...
| Component | Responsibility |
|-----------|---------------|
| `MedresAppLock` | Activity lifecycle monitoring, PIN trigger on resume |
...
| `medres_auth_prefs` | Metadata preferences: User names, API URLs, Project Names |
| `medres_auth_secure` | Secure storage: Token, Expiry, PIN Hash, Clock Validation |
...
**Implementation**: PBKDF2 hashing stored in `medres_auth_prefs`
...
- [Activities Reference](activities.md) - All MEDRES activities
...
| Module | Path |
|--------|------|
| Auth Module | `medres_auth_module/src/main/java/edu.aiims.medresodk.auth/` |
| Activities | `medres_auth_module/.../activities/` |
| Managers | `medres_auth_module/.../managers/` |
| API | `medres_auth_module/.../api/` |
