...
This document details the network monitoring and server reachability logic used by the MEDRES Auth Module to manage online/offline states and grace period re-evaluation.

## 1. Network State Monitoring
We use a **reactive** approach to monitor network connectivity rather than polling.

### `MedresNetworkStateMonitor`
The `MedresNetworkStateMonitor` (and its implementation `MedresNetworkStateMonitorImpl`) provides a `Flow<Boolean>` that emits real-time connectivity updates.
...
Merely having an internet connection doesn't guarantee the MEDRES server is reachable. Verification is done via **Real HTTP Requests**.
...
**Workflow:**
1.  **Trigger**: `MedresNetworkStateMonitor` emits `true` (Internet available).
