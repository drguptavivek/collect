# MEDRES Collect Logging & Event Strategy

> Last Updated: 2025-12-31

## Overview
This document outlines the logging and event tracking strategy for the MEDRES Auth Module. The system is designed to provide visibility into authentication flows, network health, and security incidents while maintaining strict modularity and privacy.
...
### A. Local Analytics (`MedresAppAnalytics`)
*   **Purpose**: Immediate, high-severity diagnostic logging.
*   **Implementation**: Standalone Kotlin Object (`MedresAppAnalytics`).
...
*   **Implementation**: `TelemetryWorker` + `MedresAuthManager`.
...
Tracked via `MedresAppAnalytics`.

| Event Name | Trigger | Payload |
| :--- | :--- | :--- |
| `medres_auth_login_attempt` | User taps "Login". | None |
| `medres_auth_login_success` | Valid token received. | None |
| `medres_auth_login_failed` | API returns non-200. | `reason` (e.g., "invalid_credentials_401") |
| `medres_auth_token_expired` | Token validity check fails. | None |
| `medres_auth_grace_period_started` | Offline grace period activated. | None |
| `medres_auth_hard_logout` | Grace period exceeded (6h). | `reason` |
| `medres_auth_reauth_prompt` | PIN/Biometric prompt shown. | None |
...
| Event Name | Trigger | Payload |
| :--- | :--- | :--- |
| `medres_net_error` | Exception during API call. | `type` (timeout, dns, io) |
| `medres_net_server_time_offset` | Clock synchronization. | `offset_ms` |
...
| Event Name | Trigger | Payload |
| :--- | :--- | :--- |
| `medres_sec_clock_manipulation` | Time fraud detected. | None |
| `medres_sec_manipulation_cleared` | Time corrected by user. | None |
...
| Feature | Logging (`MedresAppAnalytics`) | Telemetry (`TelemetryRequest`) |
| :--- | :--- | :--- |
...
### Telemetry Frequency
Controlled by `MedresConstants.TELEMETRY_SYNC_INTERVAL_MINUTES`.
