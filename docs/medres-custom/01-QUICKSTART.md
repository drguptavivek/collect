# MEDRES ODK Collect - Quick Start Guide

> Get up and running with MEDRES ODK Collect development
...
3. Select `medresDebug` build variant
...
1. App launches to **MedresLoginActivity**
...
```bash
adb logcat | grep -E "MedresAuth|MedresAppLock|OkHttp"
```
...
```
collect/
├── medres_auth_module/          # Custom authentication module
│   └── src/main/java/edu.aiims.medresodk.auth/
...
| Feature | Vanilla ODK | MEDRES Fork |
|---------|-------------|------------|
...
**Solution**: Ensure `google-services.json` exists for MEDRES package

### Login Crash in Release Build

**Cause**: ProGuard removed API model classes
**Solution**: Check `collect_app/proguard-rules.pro` has keep rules for `edu.aiims.medresodk.auth.api.**`
...
**Solution**: Check logs for `MedresAuthManager` cleanup events
...
| MEDRES Debug suffix | `-MEDRES-DEBUG` |
| MEDRES Release suffix | `-MEDRES` |
